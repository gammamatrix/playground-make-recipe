...
use Playground\Make\Recipe\Http\Requests\PackageModel\FormRequest;
use Playground\Make\Recipe\Http\Requests\PackageModel\SaveRequest;
use Playground\Make\Recipe\Manager;

/**
 * \Playground\Make\Recipe\Http\Controllers\PackageModelController
 */
class PackageModelController extends Controller
{
    ...
    public function delete(
        ...
        $recipe->removePackageModel($slug);
    ...
    public function form(
        ...
        if (empty($slug)) {
            $slug = $validated['model'] ?? '';
        }

        if (! empty($slug) && is_string($slug)) {
            $packageModel = $recipe->packageModel($slug);
        }

        if (empty($packageModel)) {
            $packageModel = new PackageModel($validated);
        } else {
            $packageModel->setOptions($validated);
        }

        $packageModel->apply();

        $flash = $packageModel->toArray();
         ...
         */
        $view = sprintf('%1$s::package-model/form', $packageInfo->view());
    ...
    public function save(
        ...
        $recipe->addPackageModel($slug, $packageModel);
